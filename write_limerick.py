...
def get_op(shrt):
    return "<=6" if shrt else ">=8"


def get_random_line(shrt=False):
    cursor = pg_db.cursor()
    cursor.execute(("SELECT id,tweet_id,screen_name,body,s1,s2,s3,s4 "
...
def get_rhymes(id_, s3, s4, shrt=False, limit=2):
    cursor = pg_db.cursor()
    cursor.execute(("SELECT tweet_id,screen_name,body FROM lines "
                   ...
                   .format(get_op(shrt)), (id_, s3, s4, limit))
